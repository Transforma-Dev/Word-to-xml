...
def paragraph(para, doc, doc_filename, variables, para_num, logger):
    ...
    elif ((((para.alignment == 1 and all_bold) or re.search(r"Heading\s*1", para.style.name) or space_strip.lower() == "introduction" or space_strip.strip().lower().startswith("conflict") or re.search(r'^((\d+\.*\)*\s*|\w+\.+\s*))(\w+)', para.text)) and (variables["sec_1"] == 1)) or (((para.alignment == 0 and all_bold) or re.search(r"Heading\s*2", para.style.name) or re.search(r'^\d+\.\d+(\.*|\s)+.*$', para.text)) and (variables["sec_2"] == 1)) or ((re.search(r"Heading\s*3", para.style.name) or re.search(r'^\d+\.\d+\.\d+\s.*$', para.text)) and len(para.text.split()) < 20 and variables["sec_3"] == 1)) and (not space_strip.lower().startswith("conclusion")) and len(para.text.strip()) != 0:
        xml_text = heading.heading(para, space_strip, xml_text, variables, logger)

    #Find heading in word document and change the tags sec
    elif (((para.alignment == 1 and all_bold) or re.search(r"Heading\s*1", para.style.name) or space_strip.strip().lower().startswith(("conflict","discussion","conclusion","materials"))) or ((para.alignment == 0 and all_bold)  or re.search(r"Heading\s*2", para.style.name)) or (re.search(r"Heading\s*3", para.style.name) or re.search(r'^((\b[IVX]+\.\s*|\d+(\.|\)|\s)+))(\w+)', para.text) or (all_bold and len(para.text.strip().split()) < 15))) and not re.search(r'^(Note:|Figure \d)', para.text.strip(),re.IGNORECASE) and len(para.text.split()) < 18 and len(para.text.strip()) != 0:
