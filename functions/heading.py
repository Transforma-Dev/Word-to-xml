...
def heading(para,space_strip,xml_text,variables):
    ...
    if para.alignment==1 or para.style.name.startswith("Heading 1") or space_strip.lower()=="introduction" or re.search(r'(^\d\.*\s|^\w\.*\s+)', para.text) or space_strip.strip().lower().startswith("conflict"):
        ...
        text=f'<sec id="s{variables["secid"]}_{variables["sec_3_id"]}_{variables["inner_3_id"]}"><label>{variables["secid"]}.{variables["sec_3_id"]}.{variables["inner_3_id"]}</label><title>{xml_text}</title>'
...
def sub_heading(para,xml_text,variables,space_strip):
    ...
    if para.alignment==1 or para.style.name.startswith("Heading 1") or space_strip.strip().lower().startswith("conflict") or re.search(r'^\d(\.|\s)+[A-Za-z]|^\b[IVX]+\.\s*', para.text):
        if  space_strip.strip().lower().startswith("conflict"):
            if "back" not in variables["back_start"]:
                ...
                if variables["sec_3"]>1:
                    ...
                    text=f'</sec></sec></body><back><sec id="s{variables["sec_1_id"]}"><label>{variables["sec_1_id"]}</label><title>{xml_text}</title>'
                else:
                    text=f'</sec></body><back><sec id="s{variables["sec_1_id"]}"><label>{variables["sec_1_id"]}</label><title>{xml_text}</title>'
            ...
            xml_text = re.sub('^[\d.\s*]+', '', xml_text)   #Remove numbers before string
            if variables["sec_3"]>1:
                text=f'</sec></sec></sec><sec id="s{variables["sec_1_id"]}"><label>{variables["sec_1_id"]}</label><title>{xml_text}</title>'
            elif variables["sec_2"]>1:
        ...
        xml_text = re.sub('^[\d\.\d\.\d\.*\s*]+', '', xml_text)   #Remove numbers before string
        text=f'</sec><sec id="s{variables["secid"]}_{variables["sec_3_id"]}_{variables["inner_3_id"]}"><label>{variables["secid"]}.{variables["sec_3_id"]}.{variables["inner_3_id"]}</label><title>{xml_text}</title>'
        variables["inner_3_id"]+=1

    elif para.alignment==0 or para.style.name.startswith("Heading 2") or re.search(r'^\d+\.\d+\.*\)*\s*'   , para.text):
        ...
        if "<disp-formula>" in xml_text:
            ...
            return text
        if variables["sec_3"]>1:
            text=f'</sec></sec><sec id="s{variables["secid"]}_{variables["sec_2_id"]}"><label>{variables["secid"]}.{variables["sec_2_id"]}</label><title>{xml_text}</title>'
        else:
            text=f'</sec><sec id="s{variables["secid"]}_{variables["sec_2_id"]}"><label>{variables["secid"]}.{variables["sec_2_id"]}</label><title>{xml_text}</title>'
