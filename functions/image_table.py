...
def image_caption(xml_text, variables, logger):
    ...
    try:
        ...
        if count_graphic == 0:
            ...
            if "<disp-formula" in xml_text:
                ...
                if match:
                    ...
                    figure = eq_link.add_tag(figure, logger)
        ...
        #Success log message
        logger.info(f"Successfully created the fig tag from (image_caption function) (image_table.py)-file")

        return text
    except Exception as e:
        print()
        #Error log message
        logger.error(f"Error in (image_caption function) (image_table.py)-file {e}")


# Define function to find the table heading text
def table_heading(xml_text, variables, logger):
    ...
    try:
        ...
        #Success log message
        logger.info(f"Successfully created the table-wrap tag from (table_heading function) (image_table.py)-file")
    except Exception as e:
        print(f"Error in (table_heading function) (image_table.py)-file {e}")
        #Error log message
        logger.error(f"Error in (table_heading function) (image_table.py)-file {e}")
...
def row_col_span(r, c, row, cell, table, li, tt, tr, xml_text, logger):
    ...
    try:
        ...
        print(f"Error in (row_col_span function) (image_table.py)-file {e}")
        #Error log message
        logger.error(f"Error in (row_col_span function) (image_table.py)-file {e}")
